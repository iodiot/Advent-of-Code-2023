...
function countSteps(grid, instructions, node) {
  for (let i = 0;; ++i) {
    const instruction = instructions[i % instructions.length];
    node = grid[node][instruction];
    if (node[2] === 'Z') return i + 1;
  }
}

const gcd = (a, b) => (b === 0 ? a : gcd(b, a % b));
const lcm = (a, b) => (a * b) / gcd(a, b);
const lcmArray = (arr) => arr.reduce((result, num) => lcm(result, num), arr[0]);
